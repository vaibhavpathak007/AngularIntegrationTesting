...
describe('VoterComponent', () => {
  ...
  it('should apply class highlighted', () => {
    component.myVote = 1;
    fixture.detectChanges();
    let element = fixture.debugElement.query(By.css('.glyphicon-menu-up')); // here we don't need native element
    // as we dont have to access inner html value.
    expect(element.classes['highlighted']).toBeTruthy();
  });

  it('should add vote when click on upvote', () => {
    let element = fixture.debugElement.query(By.css('.glyphicon-menu-up')); // retrieve button by class
    element.triggerEventHandler('click',null); // trigger event
